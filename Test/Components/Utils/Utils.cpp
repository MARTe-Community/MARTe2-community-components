...
bool InitialiseEnviroment(const char8 *const config, bool configure) {
  ...
  File configStream;
  EC_ASSERT_TRUE(configStream.Open(config, BasicFile::ACCESS_MODE_R));
  StandardParser parser(configStream, cdb);
  EC_ASSERT_TRUE(parser.Parse());
  ObjectRegistryDatabase *god = ObjectRegistryDatabase::Instance();
  god->Purge();
  EC_ASSERT_TRUE(god->Initialise(cdb));
  if (configure) {
    ReferenceT<RealTimeApplication> application;
    application = god->Find("Test");
    EC_ASSERT_TRUE(application.IsValid());
    EC_ASSERT_TRUE(application->ConfigureApplication());
  }
  return true;
}
...
bool StartApplication(const MARTe::char8 *const state) {
  ...
  EC_ASSERT_TRUE(application.IsValid());
  EC_ASSERT_TRUE(application->PrepareNextState(state));
  EC_ASSERT_TRUE(application->StartNextStateExecution());

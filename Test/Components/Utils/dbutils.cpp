...
bool add_signal(ConfigurationDatabase &db, const char *name, const char *type,
  ...
  EC_ASSERT_TRUE(db.MoveAbsolute("Signals"));
  EC_ASSERT_TRUE(db.CreateRelative(name));
  EC_ASSERT_TRUE(db.Write("Type", type));
  if (N > 0) {
    EC_ASSERT_TRUE(db.Write("NumberOfElements", N));
  }
  if (M > 0) {
    EC_ASSERT_TRUE(db.Write("NumberOfDimensions", M));
  }
  EC_ASSERT_TRUE(db.MoveToRoot());
  return true;
}
...
bool append_to_signal(ConfigurationDatabase &db, const char *name,
  ...
  EC_ASSERT_TRUE(db.MoveAbsolute("Signals"));
  EC_ASSERT_TRUE(db.MoveRelative(name));
  EC_ASSERT_TRUE(db.Write(property, value));
  EC_ASSERT_TRUE(db.MoveToRoot());
  return true;
}
bool set_field(ConfigurationDatabase &db, const char *field, AnyType value) {
  EC_ASSERT_TRUE(db.MoveToRoot());
  EC_ASSERT_TRUE(db.Write(field, value));
  return true;
}
...
bool add_input(ConfigurationDatabase &db, const char *name, const char *type,
  ...
  EC_ASSERT_TRUE(db.MoveAbsolute("InputSignals"));
  EC_ASSERT_TRUE(db.CreateRelative(name));
  EC_ASSERT_TRUE(db.Write("DataSource", datasource));
  EC_ASSERT_TRUE(db.Write("QualifiedName", name));
  EC_ASSERT_TRUE(db.Write("Type", type));
  EC_ASSERT_TRUE(db.Write("NumberOfElements", N));
  EC_ASSERT_TRUE(db.Write("NumberOfDimensions", M));
  EC_ASSERT_TRUE(db.MoveToRoot());
...
bool add_output(ConfigurationDatabase &db, const char *name, const char *type,
                const char *datasource, unsigned N, unsigned M) {
  EC_ASSERT_TRUE(db.MoveAbsolute("OutputSignals"));
  EC_ASSERT_TRUE(db.CreateRelative(name));
  EC_ASSERT_TRUE(db.Write("DataSource", datasource));
  EC_ASSERT_TRUE(db.Write("Type", type));
  EC_ASSERT_TRUE(db.Write("NumberOfElements", N));
